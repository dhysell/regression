...
public class TestDelinquenciesOnRenewalTerm extends BaseTest {
	...
	public void addCarryOverIfNeededAndMakeChargesDelinquent() throws Exception {
		...
		if (Calendar.getInstance().get(Calendar.DAY_OF_MONTH)%2 != 0) {
			charges.clickCarryOverChargeAndCreateChargeAndVerifyIfCreatedOrNot(myPolicyObj.accountNumber, TransactionNumber.Premium_Charged, TransactionType.Renewal, myPolicyObj.squire.getPolicyNumber(), TrueFalse.True, 300);
		}
		...
		if (Calendar.getInstance().get(Calendar.DAY_OF_MONTH)%2 != 0) {
			...
			acctMenu.clickBCMenuDelinquencies();
			Assert.assertTrue(delinquency.verifyDelinquencyByReason(OpenClosed.Open, DelinquencyReason.UnderwritingFullCancel,null,myPolicyObj.squire.getExpirationDate()),"Underwriting Full Cancel has not trigger even after receiving cancellation charges from PC");
		}
	}
