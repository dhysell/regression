 ...
 */
@QuarantineClass
public class TestFullCancellationDocumentsWithStdFireAndStdLiabilityRenewalNSF extends BaseTest {
...
//                .build(GeneratePolicyType.PolicyIssued);
    ...
    public void payAndReverseRenewalDownPaymentAndMoveClocks() throws Exception {
        ...
        combinedDownPaymentAmount = acctInvoice.getInvoiceDueAmountByRowNumber(2);
        BCTopMenu topMenu = new BCTopMenu(driver);
        topMenu.clickDesktopTab();
        ...
        //payment.reversePayment(combinedDownPaymentAmount , null, null, PaymentReversalReason.Processing_Error_Did_Not_Go_To_Bank);
    }

    @Test(dependsOnMethods = {"payAndReverseRenewalDownPaymentAndMoveClocks"})
    ...
    private void testvalidateNSFCancelNoticeDocument() throws Exception {
        testvalidateNSFCancelNoticeDocument(myPolicyObject.standardFire.getPolicyNumber());
        testvalidateNSFCancelNoticeDocument(myPolicyObject.standardLiability.getPolicyNumber());
    }

    private void testvalidateNSFCancelNoticeDocument(String PolicyNumber) throws Exception {
        Config cf = new Config(ApplicationOrCenter.PolicyCenter);
        driver = buildDriver(cf);
        uw = UnderwritersHelper.getUnderwriterInfoByLineAndTitle(Underwriter.UnderwriterLine.Personal, Underwriter.UnderwriterTitle.Underwriter_Supervisor);
