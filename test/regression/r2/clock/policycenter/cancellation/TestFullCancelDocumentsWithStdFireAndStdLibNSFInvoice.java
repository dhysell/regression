 ...
 */
@QuarantineClass
public class TestFullCancelDocumentsWithStdFireAndStdLibNSFInvoice extends BaseTest {
	private WebDriver driver;
	private ARUsers arUser;	
	private Underwriters uw;
	private Date targetDate = null;
    private GeneratePolicy myPolicyObject;
	private double combinedDownPaymentAmount , combinedInvoicePaymentAmount = 0.00;
...
//				.build(GeneratePolicyType.PolicyIssued);	
	...
	public void NonpayDownPaymentInvoiceAndMoveClocks() throws Exception {
		...
		combinedInvoicePaymentAmount = invoices.getInvoiceDueAmountByRowNumber(2);
		BCTopMenu topMenu = new BCTopMenu(driver);
		topMenu.clickDesktopTab();
		...
		payment.reversePaymentAtFault(null,combinedInvoicePaymentAmount , null, null, PaymentReturnedPaymentReason.InsufficientFunds);			
	...
	private void testValidateMidTermNonPayCancelDocument() throws Exception {
        testValidateNSFInvoiceNonPayCancelDocument(myPolicyObject.standardFire.getPolicyNumber());
        testValidateNSFInvoiceNonPayCancelDocument(myPolicyObject.standardLiability.getPolicyNumber());
	}

	private void testValidateNSFInvoiceNonPayCancelDocument(String policyNumber) throws Exception {
		Config cf = new Config(ApplicationOrCenter.PolicyCenter);
		driver = buildDriver(cf);
