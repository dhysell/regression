 ...
 * @DATE Sep 6, 2017
 */
public class TestSectionOneBlockQuoteBindIssues3 extends BaseTest {
...
//
//        Squire mySquire = new Squire(getRandomEligibility());
...
//		Configuration.setProduct(ApplicationOrCenter.PolicyCenter);
//		loginAndSearchSubmission(myPolicyObjPL.agentInfo.getAgentUserName(), myPolicyObjPL.agentInfo.getAgentPassword(),
...
//		//		propertyDetail.clickPropertyConstructionTab();
//		int yearField = DateUtils.getCenterDateAsInt(ApplicationOrCenter.PolicyCenter, "yyyy");
...
//		//		coverages.setCoverageECoverageType("Peril 1");
...
//		propertyDetail.setPropertyDetailsFA(trelliProperty);
...
//		coverages.setCoverageECoverageType("Broad Form");
//
...
//	}
	...
	public void testTrellisedHopsUWIssue()  throws Exception {
		...
		cf = new Config(ApplicationOrCenter.PolicyCenter);
		driver = buildDriver(cf);
        new Login(driver).loginAndSearchSubmission(stdFireLiab_Squire_PolicyObj.agentInfo.getAgentUserName(), stdFireLiab_Squire_PolicyObj.agentInfo.getAgentPassword(),
		...
		for (PLUWIssues uwBlockBindExpected : sqSectionIBlockBind) {
			if (!uwIssues.isInList(uwBlockBindExpected.getLongDesc()).equals(UnderwriterIssueType.AlreadyApproved)) {
				testFailed = true;
				errorMessage = errorMessage + "Expected error Bind Bind : " + uwBlockBindExpected.getShortDesc() + " is not displayed.\n";
			}
		}
