 ...
 */
@QuarantineClass
public class TestStandardFireRewriteNewTermFormInference extends BaseTest {
	...
	public void testCreateStdFireIssuance() throws Exception {
        ...
        stdFirePolObj = new GeneratePolicy.Builder(driver)
				...
				.withInsFirstLastName("Test", "New Term")
	...
	public void testStdFireRewriteNewAccountForms() throws Exception {
        ...
        new Login(driver).loginAndSearchPolicyByPolicyNumber(uw.getUnderwriterUserName(), uw.getUnderwriterPassword(), stdFirePolObj.standardFire.getPolicyNumber());
...
//		//		policyMenu.clickRewriteNewTerm();
		...
		if (this.actualExpectedDocDifferencesSubmissionPlusLocal.getInExpectedNotInUserInterface().size() > 0) {
			testfailed = true;
			errorMessage = errorMessage + " ERROR: Documents for Standard Fire Rewrite New Term In Expected But Not in UserInterface - "
	...
	private void testStdFireRewriteNewAccountDocuments() throws Exception {
		Config cf = new Config(ApplicationOrCenter.PolicyCenter);
		driver = buildDriver(cf);
		...
		String[] documents = {"Personal Lines Application",			
...
//			//			policyMenu.clickRewriteNewTerm();
			...
			if (this.actualExpectedDocDifferencesSubmissionPlusLocal.getInExpectedNotInUserInterface().size() > 0) {
				testfailed = true;
				errorMessage = errorMessage + " ERROR: Documents for Standard Fire Trellised Hops Rewrite New Term In Expected But Not in UserInterface - "
						+ this.actualExpectedDocDifferencesSubmissionPlusLocal.getInExpectedNotInUserInterface() + "\n";
			}
		...
		private void testStdFireTrellisedHopsDocuments() throws Exception {
			...
			String[] documents = {"Standard Fire Policy Declarations","Hops Property Coverage Part Value Reporting Basis",
					               "Notice of Policy on Privacy","Adverse Action Letter","Personal Lines Application"};
