 ...
 */
@QuarantineClass
public class TestStandardFireRemainderOfTermFormInference extends BaseTest {
	...
	public void testCreateStdFireBound() throws Exception {
        ...
        stdFirePolObj = new GeneratePolicy.Builder(driver)
				...
				.withInsFirstLastName("Test", "Remainder of Term")
	...
	public void testStdFireIssuanceForms() throws Exception {
		Config cf = new Config(ApplicationOrCenter.PolicyCenter);
		driver = buildDriver(cf);
	...
	@Test(dependsOnMethods = { "testStdFireIssuanceForms" })
	private void testRewriteReminderOfTerm() throws Exception {
		...
		new Login(driver).loginAndSearchPolicyByAccountNumber(uw.getUnderwriterUserName(), uw.getUnderwriterPassword(), stdFirePolObj.accountNumber);
        //Cancel Policy
        Date currentDate = stdFirePolObj.standardFire.getEffectiveDate();
        ...
        rewritePage.rewriteRemainderOfTerm(stdFirePolObj.lineSelection, ProductLineType.StandardFire);

	}

	@Test(dependsOnMethods = { "testRewriteReminderOfTerm" })
	private void testValidateStdFireRemainderOfTermDocuments() throws Exception {
		Config cf = new Config(ApplicationOrCenter.PolicyCenter);
		driver = buildDriver(cf);
		uw = UnderwritersHelper.getUnderwriterInfoByLineAndTitle(Underwriter.UnderwriterLine.Personal,
				Underwriter.UnderwriterTitle.Underwriter_Supervisor);
		...
		String[] documents = {"Property Policy Booklet","Notice of Policy on Privacy", 			
				"Adverse Action Letter","Special Endorsement for Property",				
				"Actual Cash Value Limitation for Roofing Endorsement",				
				"Specified Property Exclusion Endorsement",			
				"Cosmetic Roof Damage Endorsement","Condo Unit Endorsement", 	
				"Limited Fungi, Wet or Dry Rot, or Bacteria Endorsement","Limited Roof Coverage Endorsement",			
				"Coverage E Broad Form Endorsement","Modular Mobile Home Endorsement - Excluding Peril 17",			
				"Mobile Home Endorsement","Open Flame Warranty Endorsement"};
	...
	private void testHopsRewriteReminderOfTerm() throws Exception {
		Config cf = new Config(ApplicationOrCenter.PolicyCenter);
		driver = buildDriver(cf);
		uw = UnderwritersHelper.getUnderwriterInfoByLineAndTitle(Underwriter.UnderwriterLine.Personal,
				Underwriter.UnderwriterTitle.Underwriter_Supervisor);
        new Login(driver).loginAndSearchPolicyByPolicyNumber(uw.getUnderwriterUserName(), uw.getUnderwriterPassword(), stdFireLiab_Squire_PolicyObj.standardFire.getPolicyNumber());
        //Cancel Policy
        Date currentDate = stdFireLiab_Squire_PolicyObj.standardFire.getEffectiveDate();
		Date cancellationDate = DateUtils.dateAddSubtract(currentDate, DateAddSubtractOptions.Day, 2);
        StartCancellation cancelPol = new StartCancellation(driver);
        ...
        StartRewrite rewritePage = new StartRewrite(driver);
        rewritePage.rewriteRemainderOfTerm(stdFireLiab_Squire_PolicyObj.lineSelection, ProductLineType.StandardFire);
	}		

	@Test(dependsOnMethods = {"testHopsRewriteReminderOfTerm"})
	private void testStdFireTrellisedHopsRemainderOfTermDocuments() throws Exception {
		Config cf = new Config(ApplicationOrCenter.PolicyCenter);
		driver = buildDriver(cf);
		...
		String[] documents = {"Hops Property Coverage Part Value Reporting Basis",
				"Notice of Policy on Privacy","Adverse Action Letter","Property Policy Booklet"};
