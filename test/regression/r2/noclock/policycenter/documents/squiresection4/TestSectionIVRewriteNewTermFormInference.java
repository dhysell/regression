 ...
 */
@QuarantineClass
public class TestSectionIVRewriteNewTermFormInference extends BaseTest {
	...
	public void testSectionIVRewriteNewTermForms() throws Exception {
		Config cf = new Config(ApplicationOrCenter.PolicyCenter);
		driver = buildDriver(cf);
...
//		propertyLocations.selectRangeDirection(townshipRange.getRangeDirection());
		...
		if (this.actualExpectedDocDifferencesSubmissionPlusLocal.getInExpectedNotInUserInterface().size() > 0) {
			testfailed = true;
			errorMessage = errorMessage + " ERROR: Documents for SectionIV Rewrite New Term In Expected But Not in UserInterface - "
	...
	@Test(dependsOnMethods = {"testSectionIVRewriteNewTermForms"})
	private void testSectionIVRewriteNewTermDocuments() throws Exception {
		Config cf = new Config(ApplicationOrCenter.PolicyCenter);
		driver = buildDriver(cf);
