 ...
 */
@QuarantineClass
public class TestSectionIVRewriteRemainderOfTermFormInference extends BaseTest {
...
//		propertyLocations.selectRangeDirection(townshipRange.getRangeDirection());
	...
	private void testRewriteReminderOfTerm() throws Exception {
        ...
        Date currentDate = squireIMPolObj.squire.getEffectiveDate();
		...
		rewritePage.rewriteRemainderOfTerm(squireIMPolObj.lineSelection, ProductLineType.Squire);
		
	}

	@Test(dependsOnMethods = {"testRewriteReminderOfTerm"})
	private void testSquireRemainderOfTermDocuments() throws Exception {
		Config cf = new Config(ApplicationOrCenter.PolicyCenter);
		driver = buildDriver(cf);
		uw = UnderwritersHelper.getUnderwriterInfoByLineAndTitle(Underwriter.UnderwriterLine.Personal,
				Underwriter.UnderwriterTitle.Underwriter_Supervisor);
		...
		String[] documents = {"Certificate of Liability Insurance",
			    "Farm & Ranch Squire Policy Booklet",
