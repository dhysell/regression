 ...
 */
@QuarantineClass
public class TestSectionIVCancellationReinstateFormInference extends BaseTest {
	...
	public void testSectionIVIssuanceForms() throws Exception {
		Config cf = new Config(ApplicationOrCenter.PolicyCenter);
		driver = buildDriver(cf);
...
//		propertyLocations.selectRangeDirection(townshipRange.getRangeDirection());
		...
		ArrayList<IMFarmEquipmentScheduledItem> farmEquip = new ArrayList<IMFarmEquipmentScheduledItem>();
		farmEquip.add(farmThing);
		FarmEquipment imFarmEquip = new FarmEquipment(IMFarmEquipmentType.CircleSprinkler, CoverageType.SpecialForm, IMFarmEquipmentDeductible.FiveHundred, true, false, "Cor Hofman", farmEquip);
		farmEquipment.addFarmEquipWithAdditionalInterst(squireIMPolObj.basicSearch, imFarmEquip, AdditionalInterestType.LessorPL);
        ...
        farmEquipment.addFarmEquipmentScheduledItemExistingAdditionalInterest(farmEquipment.getFarmEquipmentAdditionalInterestValues(1, "Name"), AdditionalInterestType.LessorPL);
		...
		//changing the Special Endorsement for Inland Marine 405
	...
	@Test(dependsOnMethods = { "testSectionIVIssuanceForms" })
    public void testCancelPolicyDocuments() throws Exception {
        ...
        new GuidewireHelpers(driver).logout();
		testValidateSquireSectionIVDocuments("Cancel");	
	}
    ...
    private void testReinstatePolicyDocuments() throws Exception {
        ...
        new GuidewireHelpers(driver).logout();
		testValidateSquireSectionIVDocuments("Reinstate");
	}

	private void testValidateSquireSectionIVDocuments(String jobType) throws Exception {
		Config cf = new Config(ApplicationOrCenter.PolicyCenter);
		driver = buildDriver(cf);
		...
		String[] documents = {"Certificate of Liability Insurance",
				"Additional Interest Declarations", "Farm & Ranch Squire Policy Booklet", "Farm & Ranch Policy Declarations",
				"Notice of Policy on Privacy",
				"Adverse Action Letter",
				"Special Endorsement for Inland Marine",
