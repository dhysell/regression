 ...
 */
@QuarantineClass
public class TestSectionIVRenewalFormInference extends BaseTest {
	...
	public void testSectionIVRenewalForms() throws Exception {
		Config cf = new Config(ApplicationOrCenter.PolicyCenter);
		driver = buildDriver(cf);
		...
		preRenewalPage.closePreRenewalExplanations(squireIMPolObj);

        SearchOtherJobsPC searchJob = new SearchOtherJobsPC(driver);
		searchJob.searchJobByAccountNumber(squireIMPolObj.accountNumber, "003");
...
//		propertyLocations.selectRangeDirection(townshipRange.getRangeDirection());
	...
	@Test(dependsOnMethods = {"testSectionIVRenewalForms"})
	private void testSectionIVRenewalDocuments() throws Exception {
		Config cf = new Config(ApplicationOrCenter.PolicyCenter);
		driver = buildDriver(cf);
