 ...
 */
@QuarantineClass
public class TestSectionIVRewriteNewAccountFormInference extends BaseTest {
	private WebDriver driver;
	private GeneratePolicy squireIMPolObj, myPolicyObjPL;
	...
	public void testSectionIVRewriteNewAccountForms() throws Exception {
		Config cf = new Config(ApplicationOrCenter.PolicyCenter);
		driver = buildDriver(cf);
		...
		myPolicyObjPL = createStdLiabilityPolicy(GeneratePolicyType.PolicyIssued);
		driver.quit();
		...
		rewritePage.startQuoteRewrite(squireIMPolObj.accountNumber, myPolicyObjPL.accountNumber );

        SearchOtherJobsPC jobSearchPC = new SearchOtherJobsPC(driver);
		jobSearchPC.searchJobByAccountNumberSelectProduct(myPolicyObjPL.accountNumber, ProductLineType.Squire);		
...
//		propertyLocations.selectRangeDirection(townshipRange.getRangeDirection());
		...
		search.searchAddressBookByCompanyName(myPolicyObjPL.basicSearch, loc2Bldg1AddInterest.getCompanyName(), null, null, State.Idaho, null, CreateNew.Do_Not_Create_New);
	...
	@Test(dependsOnMethods = {"testSectionIVRewriteNewAccountForms"})
	private void testSectionIVRewriteNewAccountDocuments() throws Exception {
		Config cf = new Config(ApplicationOrCenter.PolicyCenter);
		driver = buildDriver(cf);
	...
	private GeneratePolicy createStdLiabilityPolicy(GeneratePolicyType policyType) throws Exception {
		Config cf = new Config(ApplicationOrCenter.PolicyCenter);
		driver = buildDriver(cf);
