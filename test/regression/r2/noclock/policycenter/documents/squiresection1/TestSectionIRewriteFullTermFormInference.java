 ...
 */
@QuarantineClass
public class TestSectionIRewriteFullTermFormInference extends BaseTest {
	...
	private void testRewriteFullTermAddSectionICoverages() throws Exception {
        ...
        rewritePage.rewriteFullTermGuts(mySqSectionIFRPol);
		...
		if (this.actualExpectedDocDifferencesSubmissionPlusLocal.getInUserInterfaceNotInExpected().size() > 0) {
			testfailed = true;
			errorMessage = errorMessage + "ERROR: Documents for Rewrite Full Term In UserInterface But Not in Expected - "
	...
	@Test(dependsOnMethods = { "testRewriteFullTermAddSectionICoverages" })
