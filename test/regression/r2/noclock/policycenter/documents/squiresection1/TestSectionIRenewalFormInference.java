 ...
 */
@QuarantineClass
public class TestSectionIRenewalFormInference extends BaseTest {
	...
	public void testInitiateRenewal() throws Exception {
		...
		preRenewalPage.closePreRenewalExplanations(mySqSectionIFRPol);
    }

	@Test(dependsOnMethods = { "testInitiateRenewal" })
	private void testRenewalAddSectionICoverages() throws Exception {
		Config cf = new Config(ApplicationOrCenter.PolicyCenter);
		driver = buildDriver(cf);
		...
		//As per DE5568
		//this.eventsHitDuringSubmissionCreationPlusLocal.add(DocFormEvents.PolicyCenter.Squire_CommoditySheet);
		...
		if(this.squireType.equals(SquireEligibility.City)){
			...
			this.eventsHitDuringSubmissionCreationPlusLocal.add(DocFormEvents.PolicyCenter.Squire_Liab_CountrySquirePolicyDeclarations);
			this.eventsHitDuringSubmissionCreationPlusLocal.add(DocFormEvents.PolicyCenter.Squire_Liab_LimitedPollutionCoverageEndorsement);
		}else{
		...
		if (this.actualExpectedDocDifferencesSubmissionPlusLocal.getInUserInterfaceNotInExpected().size() > 0) {
			testfailed = true;
			errorMessage = errorMessage + "ERROR: Documents for Renewal In UserInterface But Not in Expected - "
	...
	@Test(dependsOnMethods = { "testRenewalAddSectionICoverages" })
