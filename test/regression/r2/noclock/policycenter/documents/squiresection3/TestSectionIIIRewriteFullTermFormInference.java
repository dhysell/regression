...
@QuarantineClass
public class TestSectionIIIRewriteFullTermFormInference extends BaseTest {
	private WebDriver driver;
	private GeneratePolicy myPolicyObjPL;
	private String insFirstName = "RewriteFull";
	...
	public void testInitiateRewriteFullTerm() throws Exception {
		Config cf = new Config(ApplicationOrCenter.PolicyCenter);
		driver = buildDriver(cf);
	...
	@Test(dependsOnMethods = { "testInitiateRewriteFullTerm" })
	private void testRewriteFullTermAddSectionIIICoverages() throws Exception {
        ...
        rewritePage.rewriteFullTermGuts(myPolicyObjPL);
		...
		this.eventsHitDuringSubmissionCreationPlusLocal
		.add(DocFormEvents.PolicyCenter.Squire_Auto_AutomobilePolicy);

		
		this.eventsHitDuringSubmissionCreationPlusLocal
		.add(DocFormEvents.PolicyCenter.Squire_NoticeOfPolicyOnPrivacy);

		
		this.eventsHitDuringSubmissionCreationPlusLocal
		.add(DocFormEvents.PolicyCenter.Squire_AdverseActionLetter);
		...
		if (this.actualExpectedDocDifferencesSubmissionPlusLocal.getInExpectedNotInUserInterface().size() > 0) {
			testfailed = true;
			errorMessage = errorMessage + "ERROR: Documents for Rewrite Fullterm In Expected But Not in UserInterface - "
		...
		if (this.actualExpectedDocDifferencesSubmissionPlusLocal.getInUserInterfaceNotInExpected().size() > 0) {
			testfailed = true;
			errorMessage = errorMessage + "ERROR: Documents for Rewrite Fullterm In UserInterface But Not in Expected - "
	...
	@Test(dependsOnMethods = {"testRewriteFullTermAddSectionIIICoverages"})
	private void testValidateSectionIIIDocuments() throws Exception {
		...
		String[] documents = { "Classic Car Limitation of Coverage Endorsement", 
