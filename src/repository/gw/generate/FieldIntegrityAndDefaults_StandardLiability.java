...
public class FieldIntegrityAndDefaults_StandardLiability {
    public void checkClassFieldIntegrityAndDefaults(GeneratePolicy policy) {
        ...
        if (policy.lineSelection.contains(repository.gw.enums.LineSelection.StandardLiabilityPL) && policy.stdFireLiability == true) {
            ...
            policy.standardLiability.setCreateNew(repository.gw.enums.CreateNew.Do_Not_Create_New);
        ...
        if (policy.productType.equals(ProductLineType.StandardLiability) && policy.stdIMStdFire == true) {
            if (policy.standardInlandMarine == null) {
            	Assert.fail("standardInlandMarine can't be null");
            }
            policy.standardLiability.setCreateNew(CreateNew.Do_Not_Create_New);
