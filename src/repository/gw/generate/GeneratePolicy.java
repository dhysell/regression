...
public class GeneratePolicy extends GenericWorkorder {
	...
	public repository.gw.enums.ProductLineType productType = repository.gw.enums.ProductLineType.Businessowners;
	...
	public repository.gw.enums.PaymentPlanType paymentPlanType = repository.gw.enums.PaymentPlanType.getRandom();
	public repository.gw.enums.PaymentType downPaymentType = repository.gw.enums.PaymentType.getRandom();
	...
	public repository.gw.enums.OrganizationType polOrgType = repository.gw.enums.OrganizationType.Partnership;
	public repository.gw.enums.OrganizationTypePL polOrgTypePL = OrganizationTypePL.Partnership;
	...
	public ArrayList<repository.gw.enums.LineSelection> lineSelection = new ArrayList<repository.gw.enums.LineSelection>();
	private boolean addLineOfBusiness = false;

	public ArrayList<repository.gw.enums.LineSelection> getLineSelection() {
		return lineSelection;
	}

	public void setLineSelection(repository.gw.enums.LineSelection...lineSelection) {
		ArrayList<repository.gw.enums.LineSelection> newLineSelectionList = new ArrayList<repository.gw.enums.LineSelection>();
		for(repository.gw.enums.LineSelection line : lineSelection) {
	...
	public void addLineOfBusiness(repository.gw.enums.ProductLineType lineToAdd, GeneratePolicyType typeToGenerate) throws Exception {
		...
		case StandardLiability:
			...
			lineSelection.add(repository.gw.enums.LineSelection.StandardLiabilityPL);
			break;
		}
	 ...
	 */
	public void addSquireLine(repository.gw.enums.LineSelection...lineSelection) throws Exception {
		addSquireLine(repository.gw.helpers.DateUtils.getCenterDate(getDriver(), ApplicationOrCenter.PolicyCenter) , lineSelection);
	}
	...
	public void addSquireLine(Date effectiveDate, repository.gw.enums.LineSelection...lineSelection) throws Exception {

		new repository.gw.login.Login(webDriver).loginAndSearchPolicy_asUW(this);
	...
	public void quickQuote(Builder builderStuff) throws Exception {
		...
		if(addLineOfBusiness) {
			new repository.gw.login.Login(webDriver).loginAndSearchAccountByAccountNumber(this.agentInfo.getAgentUserName(), this.agentInfo.getAgentPassword(), this.accountNumber);
			addLineOfBusiness();
		} else if(altUser) {
			new repository.gw.login.Login(webDriver).login(altUserName, altPassword);
			startNewSubmission();
		} else {
			new repository.gw.login.Login(webDriver).login(this.agentInfo.getAgentUserName(), this.agentInfo.getAgentPassword());
			if (new repository.gw.login.Login(webDriver).accountLocked() && randomAgent) {
				this.agentInfo = new repository.gw.login.Login(webDriver).loginAsRandomAgent();
			}
			startNewSubmission();
		}
...
//				fillOutBOPBuildings_QQ();
			...
			case Squire:
				...
				if (lineSelection.contains(repository.gw.enums.LineSelection.PropertyAndLiabilityLinePL)) {
					fillOutSectionsIAndII_PropertyAndLiabilityQQ();
				}

				if (lineSelection.contains(repository.gw.enums.LineSelection.PersonalAutoLinePL)) {
					fillOutPLPersonalAutoQQ();
				}

				if (lineSelection.contains(repository.gw.enums.LineSelection.InlandMarineLinePL)) {
					fillOutPLInlandMarine();
				}

				if (lineSelection.contains(repository.gw.enums.LineSelection.Membership)) {
			...
			case CPP:
				...
				if (lineSelection.contains(repository.gw.enums.LineSelection.CommercialPropertyLineCPP)) {
					fillOutCPPCommercialProperty();
				}
				if (lineSelection.contains(repository.gw.enums.LineSelection.GeneralLiabilityLineCPP)) {
					fillOutCPPGeneralLiability();
				}
				if (lineSelection.contains(repository.gw.enums.LineSelection.CommercialAutoLineCPP)) {
					fillOutCPPCommercialAuto();
				}
				if (lineSelection.contains(repository.gw.enums.LineSelection.InlandMarineLineCPP)) {
	...
	protected void fillOutMembershipMembers() throws Exception {
		...
		if (!this.productType.equals(repository.gw.enums.ProductLineType.Membership)) {
	...
	public void fullAppGuts() throws Exception {
		...
		if(productType.equals(repository.gw.enums.ProductLineType.PersonalUmbrella)) {
		...
		if (this.productType.equals(repository.gw.enums.ProductLineType.Squire) && squire.isFarmAndRanch()) {
			SideMenuPC sideMenu = new SideMenuPC(webDriver);
			...
			if (!this.lineSelection.contains(repository.gw.enums.LineSelection.PersonalAutoLinePL)) {
				lineSelection.checkPersonalAutoLine(false);
				sideMenu.waitUntilAutoSidebarLinkIsNotVisible();
			}
			if (!this.lineSelection.contains(repository.gw.enums.LineSelection.PropertyAndLiabilityLinePL)) {
				lineSelection.checkPersonalPropertyLine(false);
				sideMenu.waitUntilPropertySidebarLinkIsNotVisible();
			} else if (!this.lineSelection.contains(repository.gw.enums.LineSelection.InlandMarineLinePL)) {
		...
		case Squire:
			...
			if (!this.polOrgType.equals(repository.gw.enums.OrganizationType.Sibling)) {
		...
		case StandardFire:
			household.updatePLHouseholdMembersFA(this);
			if (lineSelection.contains(repository.gw.enums.LineSelection.StandardFirePL)) {
		...
		if(getUWIssues) {
			...
			if (this.squire.alwaysOrderCreditReport) {
				...
				if (productType.equals(repository.gw.enums.ProductLineType.Squire)) {
				...
				if (productType.equals(repository.gw.enums.ProductLineType.Businessowners)) {
			...
			if (this.squire.alwaysOrderCreditReport && this.productType.equals(repository.gw.enums.ProductLineType.Squire)) {
				SideMenuPC sideMenu = new SideMenuPC(webDriver);
	...
	public void fullApp(Builder builderStuff) throws Exception {
		...
		if (this.productType != repository.gw.enums.ProductLineType.Membership) {
			fullAppGuts();
		}
	}
	...
	public void policySubmittedGuts() throws Exception {
		...
		if(!this.productType.equals(repository.gw.enums.ProductLineType.Businessowners)){
			sideMenu.clickSideMenuForms();
		...
		} else if(!productType.equals(repository.gw.enums.ProductLineType.Membership)) {
	...
	private void setPolicyNumber() {
		...
		if (this.productType.equals(repository.gw.enums.ProductLineType.Businessowners) || this.productType.equals(repository.gw.enums.ProductLineType.CPP)) {
	...
	public void policyIssuedGuts() throws Exception {
		...
		if(!agentAutoIssue){
			if (!this.productType.equals(repository.gw.enums.ProductLineType.Membership)) {
				...
				if (!this.productType.equals(repository.gw.enums.ProductLineType.Squire) && !this.productType.equals(repository.gw.enums.ProductLineType.Membership) && !this.productType.equals(repository.gw.enums.ProductLineType.StandardFire) && !this.productType.equals(repository.gw.enums.ProductLineType.StandardLiability) && !this.productType.equals(repository.gw.enums.ProductLineType.StandardIM)) {
				...
				sideMenu = new SideMenuPC(webDriver);
				sideMenu.clickSideMenuQuote();
				quotePage.issuePolicy(repository.gw.enums.IssuanceType.NoActionRequired);
	...
	public void selectProductAndFillOutQualificationsPage() throws Exception {
		...
		if(quickBuild) {
			...
			this.accountNumber = selectProductPage.startQuoteSelectProductAndGetAccountNumber(repository.gw.enums.QuoteType.FullApplication, this.productType);
		} else {
			this.accountNumber = selectProductPage.startQuoteSelectProductAndGetAccountNumber(QuoteType.QuickQuote, this.productType);
		}

		if (!(this.productType == repository.gw.enums.ProductLineType.Businessowners || this.productType == repository.gw.enums.ProductLineType.StandardIM)) {
	...
	public void fillOutStandardFirePropertyFA(PLPolicyLocationProperty property, Property.SectionIDeductible deductible) throws GuidewireNavigationException {
		...
		if (property.getpropertyType() == Property.PropertyTypePL.ResidencePremises || property.getpropertyType() == Property.PropertyTypePL.DwellingPremises || property.getpropertyType() == Property.PropertyTypePL.VacationHome || property.getpropertyType() == Property.PropertyTypePL.CondominiumVacationHome) {
			policyForms.eventsHitDuringSubmissionCreation.add(repository.gw.enums.DocFormEvents.PolicyCenter.StdFire_AddPropertyTypeWhereYouLive);
	...
	public void updatePLPersonalAutoFA() throws Exception {
		...
		if (this.productType.equals(repository.gw.enums.ProductLineType.Squire) && this.squire.isFarmAndRanch()) {
			sideMenu.clickSideMenuPACoverages();
			GenericWorkorderSquirePACoverages coverages = new GenericWorkorderSquirePACoverages(webDriver);
			coverages.fillOutSquireAutoCoverages(this);
		}
	...
	public static class Builder {
		...
		private ArrayList<repository.gw.enums.ProductLineType> partsToBuild = new ArrayList<repository.gw.enums.ProductLineType>();
		...
		public repository.gw.enums.CreateNew createNew = Create_New_Always;
		public boolean handleBlockSubmit = true;
		public boolean handleBlockSubmitRelease = true;
		...
		public ArrayList<PolicyInfoAdditionalNamedInsured> aniList = new ArrayList<PolicyInfoAdditionalNamedInsured>();
		public repository.gw.enums.MembershipType membershipType = repository.gw.enums.MembershipType.Associate;
		public List<Contact> additionalMembersToAddToMembershipList = new ArrayList<Contact>();
		...
		private Builder effectiveDate(WebDriver driver, int days) {
			return withPolEffectiveDate(repository.gw.helpers.DateUtils.dateAddSubtract(repository.gw.helpers.DateUtils.getCenterDate(driver, ApplicationOrCenter.PolicyCenter), repository.gw.enums.DateAddSubtractOptions.Day, days));
		}
		...
		public Builder withBusinessownersLine() throws Exception {
			this.partsToBuild.add(repository.gw.enums.ProductLineType.Businessowners);
		...
		public Builder withBusinessownersLine(PolicyBusinessownersLine boLine) throws GuidewireException {
			...
			this.productType = repository.gw.enums.ProductLineType.Businessowners;
		...
		public Builder withSquirePolicyUsedForUmbrella(GeneratePolicy squirePolicyUsedForUmbrella) {
			this.partsToBuild.add(repository.gw.enums.ProductLineType.PersonalUmbrella);
		...
		public Builder withLineSelection(repository.gw.enums.LineSelection...lineSelection) {
			ArrayList<repository.gw.enums.LineSelection> foo = new ArrayList<repository.gw.enums.LineSelection>();
		...
		public Builder withSquireEligibility(repository.gw.enums.SquireEligibility squireEligibility) {
		...
		public Builder withInsFirstLastName(String insFirstName, String insLastName) {
			...
			this.pniContact.setPersonOrCompany(repository.gw.enums.ContactSubType.Person);
			return this;
		}

		public Builder withInsCompanyName(String insCompanyName) {
			this.pniContact.setCompanyName(insCompanyName);
			this.pniContact.setPersonOrCompany(repository.gw.enums.ContactSubType.Company);
			return this;
		}
		...
		public Builder withInsPersonOrCompanyRandom(String insFirstName, String insLastName, String insCompanyName) {
			this.pniContact.setPersonOrCompany(new Random().nextBoolean() ? repository.gw.enums.ContactSubType.Company : repository.gw.enums.ContactSubType.Person);
		...
		public Builder withPolOrgType(repository.gw.enums.OrganizationType polOrgType) {
		...
		private void withSiblingStuff(GeneratePolicy squirePolicyUsedForSibling, String squirePolicyNumberUsedForSibling) {
			...
			this.pniContact.setDob(webDriver, repository.gw.helpers.DateUtils.dateAddSubtract(currentSystemDate, repository.gw.enums.DateAddSubtractOptions.Year, -20));
