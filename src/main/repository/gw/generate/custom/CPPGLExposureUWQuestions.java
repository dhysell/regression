...
public class CPPGLExposureUWQuestions {
	...
	private CPPGLExposureUWQuestions dependentOnQuestion = null;
	...
	private List<CPPGLExposureUWQuestions> childrenQuestions = null;
	...
	public CPPGLExposureUWQuestions(GLUnderwriterQuestions question) throws Exception {
		...
		this.questionText = question.getQuestionLabel().trim(); //replace("\"", "").trim();
		this.choiceOptions = getChoiceOptions(question);
		this.requiredAt = (question.getRequiredAt().equals("QQ") || question.getRequiredAt().equals("OR")) ? repository.gw.enums.GeneratePolicyType.QuickQuote : repository.gw.enums.GeneratePolicyType.FullApp;
		this.dependentOnQuestion = (question.getDependentOnQuestion().equals("-")) ? null : new CPPGLExposureUWQuestions(UWQuestionsHelper.getUWQuestionsQuestionParentCode(question.getParentQuestionCode()), false, false);
		...
		this.failureMessage = question.getFailureMessage().trim(); //replace("\"", "").trim();
		...
		List<CPPGLExposureUWQuestions> questionsToAdd = new ArrayList<>();
		List<GLUnderwriterQuestions> glUnderwriterQuestions = UWQuestionsHelper.getChildrenQuestions(this.questionCode);
		
		for(GLUnderwriterQuestions glQuestion : glUnderwriterQuestions) {
			questionsToAdd.add(new CPPGLExposureUWQuestions(glQuestion, true, true));
		}
		this.childrenQuestions = questionsToAdd;
	}
	
	public CPPGLExposureUWQuestions(GLUnderwriterQuestions question, boolean createParent, boolean getChildQuestions) throws Exception {
		this.classCode = Integer.valueOf(question.getClassCode());
		this.classCodeName = question.getClassCodeName();
		this.questionSequence = Integer.valueOf(question.getQuestionSequence());
		this.questionsType = repository.gw.enums.QuestionType.valueOf(question.getQuestionsType());
		...
		this.requiredAt = (question.getRequiredAt().equals("QQ") || question.getRequiredAt().equals("OR")) ? repository.gw.enums.GeneratePolicyType.QuickQuote : repository.gw.enums.GeneratePolicyType.FullApp;
		if (createParent) {
			this.dependentOnQuestion = (question.getDependentOnQuestion().equals("-")) ? null : new CPPGLExposureUWQuestions(UWQuestionsHelper.getUWQuestionsQuestionParentCode(question.getParentQuestionCode()), true, false);
		}
		this.dependentOnAnswer = question.getDependentOnAnswer();
		this.handleIncorrectAnswer = question.getHandleIncorrectAnswer().equals("Yes");
		this.correctAnswer = setCorrectAnswerString(question.getCorrectAnswer());
...
//		intList.remove(0);
	...
	private String setCorrectAnswerString(String answer) {
		if(answer.equals("-")) {
			...
			} else if(this.questionsType.equals(repository.gw.enums.QuestionType.Boolean)){
				return "No";
			}
		}
		return answer;
	...
	public CPPGLExposureUWQuestions(int classCode, String classCodeName, int questionSequence, repository.gw.enums.QuestionType questionsType, String questionLabel, List<String> choiceOptions, repository.gw.enums.GeneratePolicyType requiredAt, CPPGLExposureUWQuestions dependentOnQuestion, String dependentOnAnswer, boolean handleIncorrectAnswer, String correctAnswer, String failureMessage, repository.gw.enums.BlockingAction blockingAction) {
	...
	public CPPGLExposureUWQuestions getDependentOnQuestion() {
		return dependentOnQuestion;
	}
	public void setDependentOnQuestion(CPPGLExposureUWQuestions dependentOnQuestion) {
		this.dependentOnQuestion = dependentOnQuestion;
	}
	...
	public List<CPPGLExposureUWQuestions> getChildrenQuestions() {
		return childrenQuestions;
	}



	public void setChildrenQuestions(List<CPPGLExposureUWQuestions> childrenQuestions) {
